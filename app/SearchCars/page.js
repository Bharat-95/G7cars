...
const Page = () => {
  ...
  const dropoffDateTimeParam = searchParams.get('dropoffDateTime');
  const pickupDateTime = pickupDateTimeParam ? new Date(pickupDateTimeParam) : null;
  const dropoffDateTime = dropoffDateTimeParam ? new Date(dropoffDateTimeParam) : null;

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("https://pvmpxgfe77.execute-api.us-east-1.amazonaws.com/cars");
        if (!response.ok) {
  ...
  const confirmBooking = () => {
    ...
    if (isSignedIn) {
      setBookedCars((prevBookedCars) => ({ ...prevBookedCars, [selectedCar.G7cars123]: true }));
      router.push('payment');
    } else {
          ...
          <div key={car.G7cars123} className="m-20 cursor-pointer">
            <div className="h-96 w-64 duration-500 p-4 rounded-xl hover:border-[2px] hover:border-rose-950 space-y-4">
              ...
              <button
                ...
                className={`flex justify-center py-4 px-2 bg-rose-900 text-white rounded-md hover:bg-rose-900/95 w-52 ${bookedCars[car.G7cars123] ? "opacity-50 cursor-not-allowed" : ""}`}
                disabled={bookedCars[car.G7cars123]}
              >
                {bookedCars[car.G7cars123] ? "Car Booked" : "Rent Car"}
              </button>
            </div>
          </div>
        ))}
